...
def toggle_payment(order_id):
    ...
    try:
        ...
        logging.info(f"Payment status toggled for order {order_id}. New status: {'Paid' if order.paid else 'Unpaid'}")
        return jsonify({"message": "Payment status toggled successfully", "paid": order.paid}), 200
    
    except Exception as e:
        logging.error(f"Error toggling payment status for order {order_id}: {str(e)}")
        session.rollback()
